...
difficulty: medium
languages: ["Go"]
complexity: { time: "O(n)", space: "O(n)", timeExplanation: "You convert the linked list to an array in O(n), then recursively build the BST by visiting each element once. Total work done across all recursive calls is linear.", spaceExplanation: "O(n) to store the array converted from the linked list, plus O(log n) recursion stack space. Tree nodes also use O(n) memory, which is required output space." }
