...
difficulty: medium
languages: ["Go"]
complexity: { time: "O(n^2 * log n)", space: "O(n^2)", timeExplanation: "Iterate over all pairs (i, j), then use a hash map lookup for the third number. The result deduplication uses string keys, so constant insertion/checks, but overall loop is O(n^2). Sorting each triplet is O(log 3) = O(1).", spaceExplanation: "Hash map for number indices and a set for deduplicating triplets. In worst case, could store up to O(n^2) unique triplets." }
