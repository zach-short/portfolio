...
difficulty: easy
languages: ["Go"]
complexity: { time: "O(n * k)", space: "O(k)", timeExplanation: "Loop through each word, and for each, we perform a frequency map comparison (O(k) time, k is the max word length).", spaceExplanation: "Each word comparison uses a hash map of size at most O(k) for the 26 lowercase letters." }
