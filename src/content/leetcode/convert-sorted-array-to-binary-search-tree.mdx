...
difficulty: easy
languages: ["Go"]
complexity: { time: "O(n)", space: "O(n)", timeExplanation: "Every node must be visited atleast once. Splitting the array, and creating the header are both constant operations.", spaceExplanation: "Every array element becomes one node → n nodes → O(n) space. Depth is O(log n) for a balanced BST → O(log n) extra space. The array operation dominates. So O(n)" }
