...
difficulty: medium
languages: ["Go"]
complexity: { time: "O(n^2 * k)", space: "O(n * k)", timeExplanation: "Check every pair of strings in the worst case, and isAnagram takes O(k) time per comparison.", spaceExplanation: "Store the groups and use a map for character counting, which takes up O(n * k) space." }
...
<EdgeCases items={[`["a", "b", "c", "a"] → duplicates must be grouped together.`]} />
