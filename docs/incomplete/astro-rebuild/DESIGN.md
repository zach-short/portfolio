...
| G20 | The header nav's "About" link points at `#contact`; there is no About section. **Re-verified 2026-09-16** against the rebuilt site: `find src/pages -iname '*about*'` and `grep -rin about src/pages src/components` both return nothing; `src/components/Nav.astro:10` still points "About" at `#contact` | this session (2026-09-15), re-verified this session (2026-09-16) | `app/page.tsx:21-23` (original, file since deleted by `HANDOFF` step 9); `src/components/Nav.astro:2-3`, `:10` (current) |
